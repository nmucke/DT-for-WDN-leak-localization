...
def main():
    ...
    plt.figure()
    for j, num_sensors in enumerate(NUM_SENSORS_LIST):
        plt.plot(NOISE_LIST, topological_distance_mean[j], linewidth=2, label=f"{num_sensors} sensors")
    plt.xlabel("Noise")
    ...
    plt.figure()
    for j, num_sensors in enumerate(NUM_SENSORS_LIST):
        plt.plot(NOISE_LIST, accuracy_list[j], linewidth=2, label=f"{num_sensors} sensors")
    plt.xlabel("Noise")
    plt.ylabel("Accuracy [%]")
    plt.grid()
    plt.legend()
